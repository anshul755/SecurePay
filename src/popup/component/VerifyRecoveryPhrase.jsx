...
export default function VerifyRecoveryPhrase() {
  ...
  // Derive a hashed key using SHA-256 (must match encryption)
  const hashedKey = CryptoJS.SHA256(secretKey).toString();

  // Retrieve the encrypted mnemonic from localStorage.
  ...
  const verifyWords = () => {
    ...
    if (valid) {
      ...
      navigate("/generatepublickey"); // Replace with your actual next route.
    } else {
